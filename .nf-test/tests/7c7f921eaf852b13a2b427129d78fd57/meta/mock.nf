...
include { GUNZIP_REFFASTA } from '/home/thrillal/BilalsGituation/NF/pathogenepidemiology/nf-core-pathogenepidemiology/modules/local/gunzip_reffasta/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = file(
                    params.modules_testdata_base_path + 'genomics/sarscov2/genome/genome.fasta.gz',
                    checkIfExists: true
                )
    ...
    //run process
    GUNZIP_REFFASTA.run(input.toArray())

    if (GUNZIP_REFFASTA.output){

        // consumes all named output channels and stores items in a json file
        GUNZIP_REFFASTA.out.getNames().each { name ->
            serializeChannel(name, GUNZIP_REFFASTA.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = GUNZIP_REFFASTA.out as List<Object>
        def i = 0
        array.each { output ->

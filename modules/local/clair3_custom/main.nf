process CLAIR3_CUSTOM {
    ...
    input:
    // , path(user_model) changed to val(user_model) so null value can be passed
    // to use a locally-stored user-defined clair3 model, please use the CLAIR3 process included in main
    tuple val(meta), path(bam), path(bai), val(packaged_model), val(user_model), val(platform)
